...
export async function validateApiKey(req) {
    try {
        ...
        const validatedKey = await ensureDailyReset(keyDoc);
        let usedToday = validatedKey.usedToday || 0;
        const dailyLimit = validatedKey.dailyLimit || 25000;
        ...
        ApiKey.findByIdAndUpdate(validatedKey._id, {
            lastUsed: new Date(),
            lastUsedIp: clientIp,
        }).exec();
