    ...
    return (
        <section className="relative py-2 px-4 overflow-hidden border-b border-white/5 bg-black/10">
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="overflow-x-auto pb-1 custom-scrollbar-premium snap-x snap-mandatory">
                        ...
                        {flashSaleData.map((item, index) => (
                            ...
                            >
                                <Link
                                    ...
                                    className="group relative block w-[120px] sm:w-[130px] bg-[var(--card)]/40 backdrop-blur-lg border border-white/5 rounded-[1rem] p-1.5 transition-all duration-500 hover:border-amber-500/30 hover:bg-amber-500/[0.04] shadow-lg"
                                >
                                    {/* Badge */}
                                    <div className="absolute top-2 left-2 z-20">
                                        <span className="text-[5px] font-black uppercase tracking-widest px-1 py-0.5 rounded-md bg-amber-500 text-black shadow-lg">
                                            {item.badge}
                                        </span>
                                    </div>

                                    {/* Image Container */}
                                    <div className="relative aspect-square rounded-[0.8rem] overflow-hidden mb-1.5 ring-1 ring-white/5 bg-black/20">
                                        <Image
                                            src={item.image}
                                            alt={item.name}
                                            fill
                                    ...
                                    <div className="space-y-0">
                                        <p className="text-[6px] font-bold text-amber-500/80 uppercase tracking-widest truncate">{item.game}</p>
                                        <h3 className="text-[9px] font-black uppercase tracking-tight text-[var(--foreground)] truncate group-hover:text-amber-500 transition-colors">
                                            {item.name}
                                        </h3>

                                        <div className="flex items-center justify-between pt-0.5">
                                            <span className="text-[11px] font-black italic text-[var(--foreground)]">
                                                {item.price}
                                            </span>
                                            <span className="text-[7px] font-bold text-[var(--muted)] line-through opacity-50 decoration-red-500/50">
                                                {item.originalPrice}
                                            </span>
                                        </div>
                                    </div>
